...
import { addCinema } from '../../../api-helpers/api-helpers';
...
const AddCinemaForm = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const result = await addCinema(inputs);

            if (result && result.success) {
                setSnackbar({
                    open: true,
                    message: result.message || 'Thêm rạp chiếu thành công!',
                    severity: 'success'
                });
                ...
                throw new Error(result?.message || 'Thêm rạp chiếu thất bại');
            }
        } catch (error) {
            console.error("Error adding cinema:", error);
            setSnackbar({
                open: true,
                message: error.message || 'Đã xảy ra lỗi khi thêm rạp chiếu',
                severity: 'error'
            });
        } finally {
            setSubmitting(false);
            ...
            <Paper elevation={0} sx={{ p: { xs: 2, md: 4 }, borderRadius: 2, bgcolor: '#f9f9f9' }}>
                ...
                <Box display="flex" gap={2} justifyContent="flex-end" mt={4}>
                    ...
                    <Button
                        ...
                        size="large"
                    >
                        {submitting ? 'Đang thêm rạp chiếu...' : 'Thêm rạp chiếu'}
                    </Button>
                </Box>
            </Paper>
