...
import { getRoomById, getAllCinemas, updateRoom } from '../../../api-helpers/api-helpers';
...
const EditRoomForm = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                setLoadingError(error.message || "Đã xảy ra lỗi khi tải dữ liệu");
            } finally {
                setLoading(false);
            }
        };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const result = await updateRoom(id, inputs);

            if (result && result.success) {
                setSnackbar({
                    open: true,
                    message: result.message || 'Cập nhật phòng chiếu thành công!',
                    severity: 'success'
                });
                ...
                throw new Error(result?.message || 'Cập nhật phòng chiếu thất bại');
            }
        } catch (error) {
            console.error("Error updating room:", error);
            setSnackbar({
                open: true,
                message: error.message || 'Đã xảy ra lỗi khi cập nhật phòng chiếu',
                severity: 'error'
            });
        } finally {
            setSubmitting(false);
                    ...
                    <Grid item xs={12} md={6}>
                        <FormControl fullWidth variant="outlined" required error={!!errors.cinema}>
                            ...
                            <Select
                                ...
                                value={inputs.cinema}
                                onChange={handleChange}
                                disabled={true} // Disable changing cinema for existing rooms
                            >
                                {cinemas.map((cinema) => (
                            ...
                            <FormHelperText>Không thể thay đổi rạp chiếu</FormHelperText>
                        </FormControl>
                    </Grid>
            ...
            <Paper elevation={0} sx={{ p: { xs: 2, md: 4 }, borderRadius: 2, bgcolor: '#f9f9f9' }}>
                ...
                <Box display="flex" gap={2} justifyContent="flex-end" mt={4}>
                    ...
                    <Button
                        ...
                        startIcon={<SaveIcon />}
                    >
                        {submitting ? 'Đang cập nhật...' : 'Cập nhật phòng chiếu'}
                    </Button>
                </Box>
            </Paper>
